...
function M.validate_c_semantics(code)
  ...
  for func, header in pairs(common_headers) do
    if code:match(func .. "%s*%(") and not code:match(header:gsub("%%", "%%")) then
  ...
  for i, line in ipairs(lines) do
    ...
    if trimmed ~= "" and not trimmed:match("^#") and not trimmed:match("^//") and not trimmed:match("^/\\*") then
      if trimmed:match("[a-zA-Z_][a-zA-Z0-9_]*%s*%(") and not trimmed:match(";%s*$") and not trimmed:match("{%s*$") then
