...
function M.execute_code(module, code, context)
  ...
  -- Default implementation - should be overridden by specific languages
...
function M.setup_environment(module, config)
  ...
  if not executable_available then
    vim.notify(string.format("Shifty: Executable check failed for language '%s'", module.metadata.name), vim.log.levels.ERROR)
    return false
  end
